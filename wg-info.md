...
Важно: эти `172.xx.30.254` — **локальные IP сервера** (к ним биндятся контейнеры nginx через `ports:`), поэтому доступ идет в **INPUT** цепочку (это не транзит/forward).
...
Клиент должен “маршрутизировать” только **свой** стенд (один IP /32).
...
AllowedIPs = 172.21.30.254/32
PersistentKeepalive = 25
```

Где:
- `<SERVER_PUBLIC_IP>` — белый IP сервера (или домен)
- `AllowedIPs = 172.21.30.254/32` — ключевой момент: клиент “видит” только один IP
...
- AllowedIPs: `172.22.30.254/32` … `172.26.30.254/32`
