## WireGuard на Ubuntu — справочник (6 интерфейсов, изоляция по командам)
...
---
### Для автоматической настройки Сети, правил IPTABLES необходимо воспользоваться скриптом wg-iptables-setup.sh (предварительно указать верные переменные).
---
...
-- разрешаем **все** только в **свою подсеть /24** (например, team1 → `172.21.30.0/24` и `172.21.31.0/24`) через **FORWARD**;
...
```bash
sudo sed -i "s#<SERVER_PRIVATE_KEY>#$(sudo cat /etc/wireguard/keys/server.key)#g" /etc/wireguard/wg{0,1,2,3,4,5}.conf
```
...
AllowedIPs = 172.21.30.0/24,172.21.31.0/24
PersistentKeepalive = 25
```

Где:
- `<SERVER_PUBLIC_IP>` — белый IP сервера
- `AllowedIPs = 172.21.30.0/24,172.21.31.0/24` — ключевой момент: клиент “видит” только свои /24 подсети
...
- AllowedIPs: `172.22.30.0/24,172.22.31.0/24` … `172.25.30.0/24,172.25.31.0/24`
